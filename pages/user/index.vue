                            ...
                            <div class="pa-3">
                              ...
                              <p class="text-h6 text-md-h5">
                                <strong>
                                  ${{
                                    (this.user_info.user &&
                                    this.user_info.user.cash
                                      ? Math.ceil(this.user_info.user.cash)
                                      : 0) | numberFormat
                                  }}
                                </strong>
                              </p>
                          ...
                          >
                            ...
                            <nuxt-link to="/user/ticket" class="pa-3">
                              ...
                              <p class="text-h5">
                                <strong>
                                  {{
                                    user_info.unreadMessages &&
                                    user_info.unreadMessages.total
                                      ? user_info.unreadMessages.total
                                      : 0
                                  }}
                                </strong>
          ...
          <v-card-title class="text-h4">
            <v-row>
              ...
              <v-col cols="6" class="text-right">
                <v-btn class="warning"> See result </v-btn>
              </v-col>
            </v-row>
          </v-card-title>
                    ...
                    <tfoot>
                      <tr bgcolor="#E5FBF7">
                        ...
                        <td class="text-center">
                          {{
                            user_info.examSuggestions &&
                            user_info.examSuggestions.total
                              ? user_info.examSuggestions.total
                              : ""
                          }}
                        </td>
...
export default {
  ...
  methods: {
    async getUserInfo() {
      ...
      this.$axios
        ...
        .then((response) => {
          ...
          this.$refs.general_info.progressData =
            this.user_info.profileCompletion;

          this.$refs["create-content-section"].$refs[
            "question-statistics"
          ].statistics = this.user_info.stats;
    ...
    initContentStats(stats) {
      //Init create content box statistics
      var objIndex = this.$refs["create-content-section"].button_list.findIndex(
        (x) => x.class === "sample_exam"
      );
      this.$refs["create-content-section"].button_list[objIndex].count =
        stats.test.total;

      var objIndex = this.$refs["create-content-section"].button_list.findIndex(
        (x) => x.class === "training_content"
      );
      this.$refs["create-content-section"].button_list[objIndex].count =
        stats.file.total;

      var objIndex = this.$refs["create-content-section"].button_list.findIndex(
        (x) => x.class === "question_answer"
      );
      this.$refs["create-content-section"].button_list[objIndex].count =
        stats.question.total;

      var objIndex = this.$refs["create-content-section"].button_list.findIndex(
        (x) => x.class === "online_exam"
      );
      this.$refs["create-content-section"].button_list[objIndex].count =
        stats.test.total;
    },
