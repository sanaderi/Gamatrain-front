    ...
    <v-card>
      ...
      <validation-observer ref="observer" v-slot="{invalid}">
        <form @submit.prevent="sendReport">
          <v-card-text>
            <validation-provider v-slot="{errors}" name="report_type" rules="required">
              <v-radio-group
            ...
            <validation-provider v-slot="{errors}" name="description" rules="required|min:25">
              <v-textarea label="Description"
                          :error-messages="errors"
                          v-model="form.message"
                          outlined hint="You must enter at least 25 characters"
                          persistent-hint/>
            </validation-provider>
          </v-card-text>
...
export default {
  ...
  methods: {
    sendReport() {
      ...
      this.$axios.$post('/api/v1/reports',
